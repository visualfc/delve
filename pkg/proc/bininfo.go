...
func (bi *BinaryInfo) loadDebugInfoMaps(image *Image, debugInfoBytes, debugLineBytes []byte, wg *sync.WaitGroup, cont func()) {
	...
	// lookup dir has gop_autogen.go
	lookupByGopFiles := func(lineInfo *line.DebugLineInfo) string {
		for _, fileEntry := range lineInfo.FileNames {
			path := fileEntry.Path
			if filepath.IsAbs(path) {
				dir, name := filepath.Split(path)
				if name == "gop_autogen.go" {
					return dir
				}
			}
		}
	...
	// check dir has gop_autogen.go
	hasGopFiles := func(dir string) bool {
		_, err := os.Stat(filepath.Join(dir, "gop_autogen.go"))
		return err == nil
	}
	...
	var mainDir string
	for _, cu := range image.compileUnits {
		if cu.lineInfo != nil && cu.name == "main" {
			for _, fileEntry := range cu.lineInfo.FileNames {
				if fileEntry.Path == "_testmain.go" {
					mainDir, _ = os.Getwd()
					break
				}
				if filepath.IsAbs(fileEntry.Path) {
					mainDir, _ = filepath.Split(fileEntry.Path)
					break
				}
			}
		}
	...
	for _, cu := range image.compileUnits {
		if cu.lineInfo != nil {
			...
			var checked bool
			for _, fileEntry := range cu.lineInfo.FileNames {
				fpath := fileEntry.Path
				if fpath != "<autogenerated>" &&
					!filepath.IsAbs(fpath) && filepath.Ext(fpath) != ".go" {
					if !checked {
						checked = true
						if cu.name == "main" {
							if hasGopFiles(mainDir) {
								gopDir = mainDir
							}
						} else {
							gopDir = lookupByGopFiles(cu.lineInfo)
							if gopDir == "" {
								if dir, err := lookupPkgDir(cu.name, mainDir); err == nil {
									if hasGopFiles(dir) {
										gopDir = dir
									}
								}
					...
					if gopDir != "" {
						...
						delete(cu.lineInfo.Lookup, fpath)
						fileEntry.Path = filepath.Join(gopDir, file)
						cu.lineInfo.Lookup[fileEntry.Path] = fileEntry
					}
				}
...
func lookupPkgDir(pkg string, workDir string) (string, error) {
	if strings.HasSuffix(pkg, "_test") {
		return lookupPkgDir(pkg[:len(pkg)-5], workDir)
	}
